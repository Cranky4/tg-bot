...
func TestGetReportWithSuccess(t *testing.T) {
	...
	repo.EXPECT().GetExpenses(ctx, model.Week, userId).Return([]*model.Expense{
		{
			Amount:   12550,
			Category: "Категория",
			Datetime: date,
...
func TestGetReportWithEmptyReport(t *testing.T) {
	...
	repo.EXPECT().GetExpenses(ctx, model.Week, userId).Return([]*model.Expense{}, nil)
...
func TestGetReportWithDBError(t *testing.T) {
	...
	repo.EXPECT().GetExpenses(ctx, model.Week, userId).Return([]*model.Expense{}, errors.New("database error"))
